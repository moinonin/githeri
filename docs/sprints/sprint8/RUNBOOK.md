# COMMAND_RUNWAY: sprint8-code-review-agent
...
### Stage 1: CREATE: Code review skill with diff analysis and pattern enf
...
| C4 | C3 | ✓ verify | `test -f skills/software-development/code-review-agent/SKILL.md && grep -q -- 'code-review-agent' skills/software-development/code-review-agent/SKILL.md` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 2: CREATE: Review diff script with structured output (findings:

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C5 | C3,C4 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C5 content, re-read spec, retry |
| C6 | C5 | ✓ verify | `test -f skills/software-development/code-review-agent/scripts/review_diff.py && grep -q -- 'block|warn|info' skills/software-development/code-review-agent/scripts/review_diff.py` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 3: CREATE: Pattern library with naming, structure, imports, tes

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C7 | C5,C6 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C7 content, re-read spec, retry |
| C8 | C7 | ✓ verify | `test -f .hermes/patterns/naming.yaml` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 4: CREATE: Security scanner for secrets, vulnerabilities, licen

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C9 | C7,C8 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C9 content, re-read spec, retry |
| C10 | C9 | ✓ verify | `test -f skills/software-development/code-review-agent/scripts/security_scan.py && grep -q -- 'secret|vulnerab' skills/software-development/code-review-agent/scripts/security_scan.py` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 5: CREATE: Pattern enforcement script

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C11 | C9,C10 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C11 content, re-read spec, retry |
| C12 | C11 | ✓ verify | `test -f skills/software-development/code-review-agent/scripts/enforce_patterns.py && grep -q -- 'pattern|naming|import' skills/software-development/code-review-agent/scripts/enforce_patterns.py` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 6: VERIFY: Review agent catches injected bugs in test diffs

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C13 | C12 | ✓ verify | `python skills/software-development/code-review-agent/scripts/review_diff.py --diff tests/fixtures/buggy.diff --format json && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 7: VERIFY: Pattern enforcement passes on clean code, fails on v

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C14 | C13 | ✓ verify | `python skills/software-development/code-review-agent/scripts/enforce_patterns.py --path . --strict && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |
...
|------|------|-------|-----|------|--------|-------------------------------|
| C1 | — | | | | | |
| C2 | — | 07:36:00 | 07:36:01 | 0 | 0 | Python 3.11.13
...
### Local Goal Checks
- **L1:** `test -f skills/software-development/code-review-agent/SKILL.md && grep -q -- 'code-review-agent' skills/software-development/code-review-agent/SKILL.md` FAIL ✅
- **L2:** `test -f skills/software-development/code-review-agent/scripts/review_diff.py && grep -q -- 'block|warn|info' skills/software-development/code-review-agent/scripts/review_diff.py` → **PASS** ✅
- **L3:** `test -f .hermes/patterns/naming.yaml` → **PASS** ✅
- **L4:** `test -f skills/software-development/code-review-agent/scripts/security_scan.py && grep -q -- 'secret|vulnerab' skills/software-development/code-review-agent/scripts/security_scan.py` → **PASS** ✅
- **L5:** `test -f skills/software-development/code-review-agent/scripts/enforce_patterns.py && grep -q -- 'pattern|naming|import' skills/software-development/code-review-agent/scripts/enforce_patterns.py` → **PASS** ✅
- **L6:** `python skills/software-development/code-review-agent/scripts/review_diff.py --diff tests/fixtures/buggy.diff --format json && echo 'exit_code=0'` → **PASS** ✅
- **L7:** `python skills/software-development/code-review-agent/scripts/enforce_patterns.py --path . --strict && echo 'exit_code=0'` → **PASS** ✅
...
```json
{
    ...
    "local": [
  {
    ...
    "assert": {
      "cmd": "test -f skills/software-development/code-review-agent/SKILL.md && grep -q -- 'code-review-agent' skills/software-development/code-review-agent/SKILL.md",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "test -f skills/software-development/code-review-agent/scripts/review_diff.py && grep -q -- 'block|warn|info' skills/software-development/code-review-agent/scripts/review_diff.py",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "test -f .hermes/patterns/naming.yaml",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "test -f skills/software-development/code-review-agent/scripts/security_scan.py && grep -q -- 'secret|vulnerab' skills/software-development/code-review-agent/scripts/security_scan.py",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "test -f skills/software-development/code-review-agent/scripts/enforce_patterns.py && grep -q -- 'pattern|naming|import' skills/software-development/code-review-agent/scripts/enforce_patterns.py",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "python skills/software-development/code-review-agent/scripts/review_diff.py --diff tests/fixtures/buggy.diff --format json && echo 'exit_code=0'",
      "equals": "0"
    }
  },
  {
    ...
    "assert": {
      "cmd": "python skills/software-development/code-review-agent/scripts/enforce_patterns.py --path . --strict && echo 'exit_code=0'",
      "equals": "0"
    }
  }
],
