...
- L1: CREATE: Add CLI flags --provider (ollama|openrouter) and --cloud-model for scoring in run_pipeline.py
- L2: VERIFY: Config in .env file for model providers
- L3: VERIFY: Tests pass with ollama provider
- L4: VERIFY: Tests pass with openrouter provider
...
### Stage 1: CREATE: Add CLI flags --provider (ollama|openrouter) and --c
...
| C4 | C3 | ✓ verify | `test -f packages/pipeline/src/run_pipeline.py && grep -q '--cloud-model' packages/pipeline/src/run_pipeline.py` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 2: VERIFY: Config in .env file for model providers

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C5 | C4 | ✓ verify | `test -f packages/pipeline/.env.example && grep -q 'OPENROUTER_API_KEY=' packages/pipeline/.env.example` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 3: VERIFY: Tests pass with ollama provider

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C6 | C5 | ✓ verify | `pnpm test --filter=@verified-attention/pipeline-model-provider-ollama && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 4: VERIFY: Tests pass with openrouter provider

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C7 | — | ⏾ inspect | `cat packages/core/src/index.ts` | file contents understood | file may not exist -- search for it, log finding |
| C8 | — | ⏾ inspect | `pnpm --version` | version string | dependency missing -- install or halt |
| C9 | C8 | ✓ verify | `pnpm test --filter=@verified-attention/pipeline-model-provider-openrouter && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |
...
|------|------|-------|-----|------|--------|-------------------------------|
| C1 | — | 23:15:18 | 23:15:18 | 1 | 0 |  | ERR: cat: packages/core/src/index.ts: No such file or d |
| C2 | — | 23:15:18 | 23:15:18 | 0 | 0 | 9.0.0 |
| C3 | C1,C2 | | | | | |
...
- **L1:** `test -f packages/pipeline/src/run_pipeline.py && grep -q '--cloud-model' packages/pipeline/src/run_pipeline.py` FAIL ✅
- **L2:** `test -f packages/pipeline/.env.example && grep -q 'OPENROUTER_API_KEY=' packages/pipeline/.env.example` → **PASS** ✅
- **L3:** `pnpm test --filter=@verified-attention/pipeline-model-provider-ollama && echo 'exit_code=0'` → **PASS** ✅
- **L4:** `pnpm test --filter=@verified-attention/pipeline-model-provider-openrouter && echo 'exit_code=0'` → **PASS** ✅
...
```json
{
  ...
  "goals": {
    "local": [      {
        ...
        "assert": {
          "cmd": "test -f packages/pipeline/src/run_pipeline.py && grep -q '--cloud-model' packages/pipeline/src/run_pipeline.py",
          "equals": "0"
        }
      },      {
        "id": "L2",
        "description": "VERIFY: Config in .env file for model providers",
        "assert": {
          "cmd": "test -f packages/pipeline/.env.example && grep -q 'OPENROUTER_API_KEY=' packages/pipeline/.env.example",
          "equals": "0"
        }
      },      {
        "id": "L3",
        "description": "VERIFY: Tests pass with ollama provider",
        "assert": {
          "cmd": "pnpm test --filter=@verified-attention/pipeline-model-provider-ollama && echo 'exit_code=0'",
          "equals": "0"
        }
      },      {
        "id": "L4",
        "description": "VERIFY: Tests pass with openrouter provider",
        "assert": {
          "cmd": "pnpm test --filter=@verified-attention/pipeline-model-provider-openrouter && echo 'exit_code=0'",
          "equals": "0"
        }
      },
    ],
