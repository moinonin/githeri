...
Expected Deliverables: CREATE: Endpoint handler for POST /api/evidence, VERIFY: Schema validation for evidence payload, VERIFY: Evidence is stored in SQLite
Dependencies: None
...
## Stage 1: Endpoint handler for POST /api/evidence

### Objective
CREATE: Endpoint handler for POST /api/evidence

### Inputs
- Spec: add-evidence-endpoint
...
### Execution Tasks
1. **CREATE**: CREATE: Endpoint handler for POST /api/evidence
...
cat > src/routes/api/evidence.ts << 'EOF'
# Implementation here
EOF

# 3. Run verification
test -f src/routes/api/evidence.ts && grep -q -- 'POST /api/evidence' src/routes/api/evidence.ts
```
...
## Stage 2: Schema validation for evidence payload

### Objective
VERIFY: Schema validation for evidence payload
...
### Execution Tasks
1. **VERIFY**: VERIFY: Schema validation for evidence payload
...
# 2. Create/Modify implementation
cat > src/schemas/evidence.schema.ts << 'EOF'
# Implementation here
EOF

# 3. Run verification
test -f src/schemas/evidence.schema.ts && grep -q -- 'payload' src/schemas/evidence.schema.ts
```
...
## Stage 3: Evidence is stored in SQLite

### Objective
VERIFY: Evidence is stored in SQLite
...
### Execution Tasks
1. **VERIFY**: VERIFY: Evidence is stored in SQLite
   - Run verification command
   - Confirm expected output
...
# 3. Run verification
pnpm test --filter=@verified-attention/evidence-storage && echo 'exit_code=0'
```
