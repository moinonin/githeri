...
- L1: CREATE: EvidenceController class with handleEvidence method
- L2: VERIFY: Schema validation works for evidence payload
- L3: VERIFY: Evidence is stored in SQLite database
...
### Stage 1: CREATE: EvidenceController class with handleEvidence method
...
| C4 | C3 | ✓ verify | `test -f packages/api/src/controllers/EvidenceController.ts && grep -q -- 'handleEvidence' packages/api/src/controllers/EvidenceController.ts` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 2: VERIFY: Schema validation works for evidence payload

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C5 | C4 | ✓ verify | `pnpm test --filter=@verified-attention/api-evidence-schema && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 3: VERIFY: Evidence is stored in SQLite database

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C6 | C5 | ✓ verify | `curl -s -o /dev/null -w '%{http_code}' -X POST  -d '"{\"content_id\": \"123\", \"session_id\": \"456\", \"payload\": {\"key\": \"value\"}}"' http://localhost:3000/api/evidence | grep -q '^201$'` | expected result, exit 0 | revert, re-prompt with error context |
...
|------|------|-------|-----|------|--------|-------------------------------|
| C1 | — | 04:49:35 | 04:49:35 | 1 | 0 |  | ERR: cat: package.json: No such file or directory |
| C2 | — | 04:49:35 | 04:49:35 | 0 | 0 | v25.1.0
9.0.0 |
| C3 | C1,C2 | | | | | |
...
### Local Goal Checks
- **L1:** `test -f packages/api/src/controllers/EvidenceController.ts && grep -q -- 'handleEvidence' packages/api/src/controllers/EvidenceController.ts` PASS
- **L2:** `pnpm test --filter=@verified-attention/api-evidence-schema && echo 'exit_code=0'` PASS
- **L3:** `curl -s -o /dev/null -w '%{http_code}' -X POST  -d '"{\"content_id\": \"123\", \"session_id\": \"456\", \"payload\": {\"key\": \"value\"}}"' http://localhost:3000/api/evidence | grep -q '^201$'` → **PASS** ✅
...
```json
{
  "task_id": "evidence-endpoint",
  "status": "Verified",
  "generated_with": ".runbookprompt.md",
  "goals": {
    "local": [      {
        "id": "evidence-endpoint",
        "description": "CREATE: EvidenceController class with handleEvidence method",
        "assert": {
          "cmd": "test -f packages/api/src/controllers/EvidenceController.ts && grep -q -- 'handleEvidence' packages/api/src/controllers/EvidenceController.ts",
          "equals": "0"
        }
      },
    ],
