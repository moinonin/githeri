...
- L2: VERIFY: CLI flags are recognized and processed correctly
- L3: CREATE: Add configuration for Ollama and OpenRouter in .env file
- L4: VERIFY: Environment variables are loaded correctly
...
| C4 | C3 | ✓ verify | `test -f src/run_pipeline.py && grep -q '--cloud-model' src/run_pipeline.py` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 2: VERIFY: CLI flags are recognized and processed correctly

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C5 | C4 | ✓ verify | `python src/run_pipeline.py --provider ollama --cloud-model model1 && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 3: CREATE: Add configuration for Ollama and OpenRouter in .env 

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C6 | — | ⏾ inspect | `cat packages/core/src/index.ts` | file contents understood | file may not exist -- search for it, log finding |
| C7 | — | ⏾ inspect | `pnpm --version` | version string | dependency missing -- install or halt |
| C8 | C6,C7 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C8 content, re-read spec, retry |
| C9 | C8 | ✓ verify | `test -f .env && grep -q 'OPENROUTER_API_KEY=' .env` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 4: VERIFY: Environment variables are loaded correctly

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C10 | C9 | ✓ verify | `python -c "import os; print(os.getenv('OLLAMA_API_KEY'))" && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |
...
|------|------|-------|-----|------|--------|-------------------------------|
| C1 | — | 22:56:00 | 22:56:00 | 1 | 0 |  | ERR: cat: packages/core/src/index.ts: No such file or d |
| C2 | — | 22:56:00 | 22:56:00 | 0 | 0 | 9.0.0 |
| C3 | C1,C2 | | | | | |
...
- **L1:** `test -f src/run_pipeline.py && grep -q '--cloud-model' src/run_pipeline.py` FAIL ✅
- **L2:** `python src/run_pipeline.py --provider ollama --cloud-model model1 && echo 'exit_code=0'` → **PASS** ✅
- **L3:** `test -f .env && grep -q 'OPENROUTER_API_KEY=' .env` → **PASS** ✅
- **L4:** `python -c "import os; print(os.getenv('OLLAMA_API_KEY'))" && echo 'exit_code=0'` → **PASS** ✅
...
```json
{
  ...
  "goals": {
    "local": [      {
        ...
        "assert": {
          "cmd": "test -f src/run_pipeline.py && grep -q '--cloud-model' src/run_pipeline.py",
          "equals": "0"
        }
      },      {
        "id": "L2",
        "description": "VERIFY: CLI flags are recognized and processed correctly",
        "assert": {
          "cmd": "python src/run_pipeline.py --provider ollama --cloud-model model1 && echo 'exit_code=0'",
          "equals": "0"
        }
      },      {
        "id": "L3",
        "description": "CREATE: Add configuration for Ollama and OpenRouter in .env file",
        "assert": {
        ...
        "id": "L4",
        "description": "VERIFY: Environment variables are loaded correctly",
        "assert": {
          "cmd": "python -c \\"import os; print(os.getenv('OLLAMA_API_KEY'))\\" && echo 'exit_code=0'",
          "equals": "0"
        }
      },
    ],
