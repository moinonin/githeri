...
|---|--------------|--------------|
| P1 | Python 3.11+, pip installed - Virtual environment active | `cat pyproject.toml || cat requirements.txt || echo 'No pyproject.toml or requirements.txt'` |
...
| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C4 | C2,C3 | ✎ create | `write_file <path>` with <content_ref> | new file on disk | revert C4 content, re-read spec, retry |
| C5 | C4 | ✓ verify | `test -f scripts/run_pipeline.py && grep -q -- '--provider' scripts/run_pipeline.py` | expected result, exit 0 | revert, re-prompt with error context |
...
| C7 | C6 | ✓ verify | `test -e scripts/run_pipeline.py` | expected result, exit 0 | revert, re-prompt with error context |
...
| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C10 | C9 | ✓ verify | `python scripts/run_pipeline.py --provider ollama --cloud-model model1 && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |

### Stage 6: VERIFY: run_pipeline.py recognizes --provider openrouter fla

| Cmd# | Deps | Type | Command / Tool Invocation | Expected Artifact / Δ | Fallback if Fail |
|------|------|------|---------------------------|------------------------|------------------|
| C11 | C10 | ✓ verify | `python scripts/run_pipeline.py --provider openrouter --cloud-model model1 && echo 'exit_code=0'` | expected result, exit 0 | revert, re-prompt with error context |
...
|------|------|-------|-----|------|--------|-------------------------------|
| C1 | — | | | | | |
| C2 | — | 04:28:33 | 04:28:33 | 0 | 0 | Python 3.11.13
...
- **L2:** `test -f scripts/run_pipeline.py && grep -q -- '--provider' scripts/run_pipeline.py` PASS
...
```json
{
  ...
  "preconditions": [
    {"id": "P1", "check": "python --version && pip --version", "expect_regex": "v(2[0-9]|3[0-9])"},
