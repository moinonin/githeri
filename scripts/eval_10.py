#!/usr/bin/env python3
"""Evaluate all 10 held-out prompts with chat format."""
import json
import sys
import subprocess
...
# All 10 held-out prompts from eval_model.py
HELD_OUT_PROMPTS = [
...
def main():
    print("📊 Evaluating 10 held-out prompts with chat format...\n")
    ...
    print("\n" + "=" * 60)
    print("EVALUATION RESULTS (10 prompts)")
    print("=" * 60)
    print(f"Base model:  Validation {base_results['validation_rate']*100:.1f}% | Score pass {base_results['score_pass_rate']*100:.1f}%")
    print(f"Fine-tuned:  Validation {ft_results['validation_rate']*100:.1f}% | Score pass {ft_results['score_pass_rate']*100:.1f}%")
    ...
    print(f"\nImprovement: Validation {val_imp:+.1f}% | Score pass {score_imp:+.1f}%")
    ...
    output = {"base_model": base_results, "fine_tuned_model": ft_results}
    out_path = PROJECT_ROOT / "data" / "eval_results_10.json"
